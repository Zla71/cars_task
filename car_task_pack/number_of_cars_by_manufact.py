...
def find_manufacturer_counts(adastra_class_instance, data_frame, time) -> None:
    ...
    try:
        ...
        write_log.write_logging(adastra_class_instance, f"{time} --- The number of cars made by each manufacturer was printed..\n\n")
        write_log.write_logging(adastra_class_instance, "Here are the numbers of cars made by manufacturer:\n\n")
        ...
        for car_brand, number_of_cars_by_brand in value_counts.items():
            if number_of_cars_by_brand < 2:
                ...
                write_log.write_logging(adastra_class_instance, f"{car_brand}: {number_of_cars_by_brand} car\n")
            else:
                print(f"{car_brand}: {number_of_cars_by_brand} cars")
                write_log.write_logging(adastra_class_instance, f"{car_brand}: {number_of_cars_by_brand} cars\n")                
